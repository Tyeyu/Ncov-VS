...
def cal_time(origin_data):  # 计算新增（地区&省级）
    ...
    for e_row in range(0, len(origin_data)):
        ...
        if ((lb == '城市级' or lb == '省级') and ex_GAT(sf) == True):
...
def write_txt_1(time_dict):
    f1.write("------------------不符合规则省级复核，每天地区新增和 == 省级新增-----------------\n")
    ...
    for i in range(0, len(time_dict)):
        ...
        for j in range(0, len(time_dict[time_keys[i]])):
            ...
            if a[0] != a[4]:
                error_inf = ShengFen[j] + ',' + time_keys[i] + ',城市级新增确诊病例累加值为' + str(int(a[0])) + ',省级新增' + str(
                    int(a[4]))
            ...
            if a[1] != a[5]:
                error_inf = ShengFen[j] + ',' + time_keys[i] + ',城市级新增治愈出院数累加值为' + str(int(a[1])) + ',省级新增' + str(
                    int(a[5]))
            ...
            if a[2] != a[6]:
                error_inf = ShengFen[j] + ',' + time_keys[i] + ',城市级新增死亡数累加值为' + str(int(a[2])) + ',省级新增' + str(
                    int(a[6]))
            ...
            if a[3] != a[7]:
                error_inf = ShengFen[j] + ',' + time_keys[i] + ',城市级新增核减累加值为' + str(int(a[3])) + ',省级新增' + str(int(a[7]))
    ...
    for i in range(0, len(cumulative_dict)):
        ...
        for j in range(0, len(cumulative_dict[cumulative_keys[i]])):
            ...
            if cu_time[j] == '2月7日':
                ...
                if a[0] != a[4]:
                    error_inf = cumulative_keys[i] + ',日期:' + cu_time[j] + ',确诊累计值' + str(int(a[0])) + ',卫健委累计值' + str(
                        int(a[4]))
                ...
                if a[1] != a[5]:
                    error_inf = cumulative_keys[i] + ',日期:' + cu_time[j] + ',出院累计值' + str(int(a[1])) + ',卫健委累计值' + str(
                        int(a[5]))
                ...
                if a[2] != a[6]:
                    error_inf = cumulative_keys[i] + ',日期:' + cu_time[j] + ',死亡累计值' + str(int(a[2])) + ',卫健委累计值' + str(
                        int(a[6]))
                ...
                if a[0] + b[4] - a[3] != a[4]:
                    error_inf = cumulative_keys[i] + ',日期:' + cu_time[j] + ',确诊累计值' + str(
                        int(a[0] + b[4] - a[3])) + ',卫健委累计值' + str(int(a[4]))
    ...
    for i in range(0, len(postion_dict)):
        ...
        for j in range(0, len(cumulative_dict[cumulative_key[i]])):
            ...
            if cu_time[j] == '1月10日':
                ...
                b = cumulative_dict[cumulative_key[i]]['1月31日']
                ...
                b = cumulative_dict[cumulative_key[i]][aa]
